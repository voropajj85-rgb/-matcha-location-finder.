...
set
  ...
  listing_type = coalesce(listing_type, case
    when lower(coalesce(status, '')) = 'lead' then 'broker_lead'
    else 'direct_listing'
  end),
  availability_status = coalesce(availability_status, case
    when lower(coalesce(status, '')) = 'lead' then 'lead'
    else 'unknown'
  end),
...
where external_id is null
   ...
   or source_family is null
   or listing_type is null
   or availability_status is null
   or unit_area is null
   or rent is null
   or provision is null
   or abloese is null
   or kaution is null
   or gastro_suitability is null
   or verified_summary is null
...
begin
  if not exists (select 1 from pg_constraint where conname = 'listings_availability_status_check') then
    alter table public.listings add constraint listings_availability_status_check
      check (availability_status in ('active', 'dead', 'unknown', 'search_only', 'lead'));
  end if;
  ...
  if not exists (select 1 from pg_constraint where conname = 'listings_listing_type_check') then
    alter table public.listings add constraint listings_listing_type_check
      check (listing_type in ('direct_listing', 'project_lead', 'broker_lead', 'municipal_lead', 'manual_lead'));
  end if;

  if not exists (select 1 from pg_constraint where conname = 'listings_gastro_suitability_check') then
    alter table public.listings add constraint listings_gastro_suitability_check
      check (gastro_suitability in ('confirmed', 'possible', 'unknown', 'no'));
  end if;
end $$;

create unique index if not exists listings_external_id_key
